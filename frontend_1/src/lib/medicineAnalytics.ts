...
export async function getDoctorMedicineSuggestions(
  ...
  try {
    console.log("getDoctorMedicineSuggestions called with:", { doctorId, complaints });
    
    // Get doctor's prescriptions from last 30 days, prioritizing recent ones
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    console.log("Looking for prescriptions after:", thirtyDaysAgo.toISOString().split('T')[0]);
    ...
    const medicinePatterns = new Map<string, {
      medicine: Medicine;
      frequency: number;
      lastPrescribed: string;
    ...
    prescriptions?.forEach((prescription: any) => {
      const prescComplaints = Array.isArray(prescription.complaints) ? prescription.complaints : [];
      ...
      const matchingComplaints = complaints.filter(complaint => 
        prescComplaints.some((pComplaint: string) => {
          ...
          return cleanComplaint.includes(cleanPrescComplaint) || cleanPrescComplaint.includes(cleanComplaint);
        })
      );
    ...
    const suggestions: MedicineSuggestion[] = Array.from(medicinePatterns.values())
      .map(pattern => {
        // Calculate confidence based on frequency, recency, and complaint relevance
        const daysSinceLastPrescribed = Math.floor(
          (Date.now() - new Date(pattern.lastPrescribed).getTime()) / (1000 * 60 * 60 * 24)
        );
        ...
        const relevanceScore = Math.min(pattern.complaintsMatch / complaints.length, 1);
        
        const confidence = (frequencyScore * 0.4 + recencyScore * 0.3 + relevanceScore * 0.3) * 100;
      ...
      .sort((a, b) => {
        ...
        return new Date(b.lastPrescribed).getTime() - new Date(a.lastPrescribed).getTime();
      })
